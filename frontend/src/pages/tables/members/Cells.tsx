...
const DataTable: React.FC = () => {
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <Table height={Math.max(getHeight(window) - 200, 400)} data={employees}>
          ...
          <Column width={250}>
            ...
            <Cell>
              {(rowData: Employee) => 
                Array.isArray(rowData.assigned_services) && rowData.assigned_services.length > 0 
      ...
      <DrawerView open={showDrawer} onClose={() => setShowDrawer(false)} {...(editingEmployee && { employeeId: editingEmployee.id })} />
    </>
  );
};
